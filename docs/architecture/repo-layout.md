...
> Current source tree. Designed to slot the Phase 2 C++/CUDA library — see [phase2_cpp.md](phase2-cpp.md) — under `cpp/` later without restructuring.
...
│   ├── sde/
│   │   ├── __init__.py
│   │   ├── base.py           # Euler–Maruyama core; see ../doc/math/euler-maruyama.md
│   │   ├── gbm.py            # see ../doc/math/gbm.md
│   │   └── heston.py         # see ../doc/math/heston.md
│   ├── risk/
│   │   ├── __init__.py
│   │   ├── var.py            # see ../doc/math/monte-carlo-var.md
│   │   └── backtest.py       # Kupiec POF, Christoffersen
│   ├── calibration/
│   │   ├── __init__.py
│   │   ├── heston_fit.py     # see ../doc/math/adjoint-sde.md
│   │   └── losses.py         # see ../doc/math/calibration-losses.md
│   ├── data/
│   │   ├── __init__.py
...
`tests/test_doc_parity.py` walks `stochastech/sde/` and `stochastech/calibration/`, asserts every `.py` (excluding `__init__.py`) has a corresponding `doc/math/*.md` whose link to the module resolves. CI runs this on push. See [Scope](../project/scope.md).
