...
| Item | Phase | Notes |
|------|-------|-------|
| C++/CUDA `neuro_ito` library | 2 | Spec preserved in [architecture/phase2_cpp.md](../architecture/phase2-cpp.md). One CUDA kernel is allowed as a Week 6 stretch and that's it. |
| ORIA portfolio integration | 3 | The live $100k ORIA fund stays untouched. See [reference/oria_inspiration.md](../project/oria-inspiration.md) for context — paper cites it as motivation only. |
| Fourier Neural Operators / PINN-FNO hybrids | 3+ | From the archived Aether-Flow vision. See [Future Work](future-work.md). |
| Transformer / GNN signal generators | 3+ | See [Future Work](future-work.md). |
...
The math doc template is documented in [math/00_index.md](../math/index.md). Required sections: Statement, Derivation, Discretization, Assumptions and failure modes, References.
