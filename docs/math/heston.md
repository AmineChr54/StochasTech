...
> Stochastic-volatility model whose differentiable calibration is the project's headline contribution. Code: [stochastech/sde/heston.py](https://github.com/AmineChr54/StochasTech/blob/main/stochastech/sde/heston.py).
...
The implementation is `simulate_heston(...)` in [stochastech/sde/heston.py](https://github.com/AmineChr54/StochasTech/blob/main/stochastech/sde/heston.py); a helper `feller_condition(kappa, theta, xi)` in the same module reports whether the calibrated parameters satisfy $2\kappa\theta \ge \xi^2$.
