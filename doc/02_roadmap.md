...
Every week with a math track ships its corresponding math docs in [math/](math/) **before or alongside** the code. A code module without its math doc is incomplete (CI enforces this — see [03_scope.md](03_scope.md)).
...
**Math docs (mandatory).**
- [math/01_brownian_motion.md](math/01_brownian_motion.md)
- [math/02_ito_calculus.md](math/02_ito_calculus.md)
- [math/03_gbm.md](math/03_gbm.md)
- [math/05_euler_maruyama.md](math/05_euler_maruyama.md)
...
**Math docs (mandatory).**
- [math/04_heston.md](math/04_heston.md) — model derivation + Feller condition
- [math/06_monte_carlo_var.md](math/06_monte_carlo_var.md) — estimator + convergence rates
...
**Math docs (mandatory).**
- [math/07_adjoint_sde.md](math/07_adjoint_sde.md)
- [math/08_calibration_losses.md](math/08_calibration_losses.md)
...
**Paper.** Draft sections — abstract, intro, math model, method, experiments, results. Pull abstract seed from [01_overview.md](01_overview.md) and equations from [math/](math/) directly.
...
**Stretch (only if ahead of schedule).** Write **one** CUDA kernel — the path-generation inner loop — behind a `--gpu` flag. Reference benchmark in paper appendix. This is the seed of the deferred [architecture/phase2_cpp.md](architecture/phase2_cpp.md) library.

**Status (2026-05-01).** Polish complete: end-to-end quickstart in [README.md](../README.md), reproducible figures via [scripts/reproduce_figures.py](../scripts/reproduce_figures.py) with helpers in [stochastech/viz/plots.py](../stochastech/viz/plots.py), CI runs lint + doc-parity + pytest under pixi ([.github/workflows/ci.yml](../.github/workflows/ci.yml)), 120 tests green. CUDA stretch deferred to Phase 2.
