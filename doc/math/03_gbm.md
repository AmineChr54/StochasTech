...
> Constant-volatility baseline model for asset prices. Code: [stochastech/sde/gbm.py](../../stochastech/sde/gbm.py).
...
Apply Itô's lemma (see [02_ito_calculus.md](02_ito_calculus.md)) to $f(s) = \log s$, with $f_s = 1/s$, $f_{ss} = -1/s^2$, $f_t = 0$. Plug into
...
The implementation is `simulate_gbm(...)` in [stochastech/sde/gbm.py](../../stochastech/sde/gbm.py); pass `log_euler=True` (default) for the exact scheme, `log_euler=False` to fall through to the generic [stochastech/sde/base.py](../../stochastech/sde/base.py) Euler–Maruyama for comparison.

## Assumptions and failure modes

- Constant $\sigma$ — empirically false (volatility clustering). Motivation for Heston (see [04_heston.md](04_heston.md)).
